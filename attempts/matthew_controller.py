...
# Dr. Scott Dick
# Matthew Lai
# Original Controller by Dr. Scott Dick modified by Matthew Lai
...
class MatthewLaiController(KesslerController):
    ...
    def __init__(self):
        ...
        """
        Matthew: Added antecedents and consequents
        """
        # Add antecedents for thrust and mines
        distance_to_asteroid = ctrl.Antecedent(np.arange(0, 1000, 1), 'distance_to_asteroid')
        thrust = ctrl.Consequent(np.arange(-480.0, 480.1, 1.0), 'thrust')

        # Define fuzzy sets for distance to asteroid
        distance_to_asteroid['Close'] = fuzz.zmf(distance_to_asteroid.universe, 0, 200)
        distance_to_asteroid['Medium'] = fuzz.trimf(distance_to_asteroid.universe, [100, 500, 800])
        distance_to_asteroid['Far'] = fuzz.smf(distance_to_asteroid.universe, 600, 1000)

        # Define fuzzy sets for thrust
        thrust['Rev'] = fuzz.trimf(thrust.universe, [-480.0, -240.0, 0.0])  # Reverse/slow down
        thrust['Stop'] = fuzz.trimf(thrust.universe, [-50.0, 0.0, 50.0])    # Minimal or no movement
        thrust['Medium'] = fuzz.trimf(thrust.universe, [0.0, 240.0, 480.0])  # Forward movement
        """
        End Edit
        """
        ...
        """
        Matthew Edit for defining rules
        """
        # Rules for thrust
        # Rule 1: If asteroid is far and few nearby, move at high thrust
        rule_thrust_high = ctrl.Rule(distance_to_asteroid['Far'], thrust['Medium'])

        # Rule 2: If asteroid is at medium distance and some are nearby, moderate thrust
        rule_thrust_medium = ctrl.Rule(distance_to_asteroid['Medium'], thrust['Medium'])

        # Rule 3: If asteroid is close or many nearby, slow down or stop
        rule_thrust_reverse = ctrl.Rule(distance_to_asteroid['Close'], thrust['Rev'])
        """
        End Edit
        """
        ...
        self.thrust_control = ctrl.ControlSystem([rule_thrust_high, rule_thrust_medium, rule_thrust_reverse])
